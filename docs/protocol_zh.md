...
| 1–4 | `linear_x` | `float32` | 底盘前后速度（m/s） |
| 5–8 | `linear_y` | `float32` | 底盘左右速度（m/s） |
| 9–12 | `linear_z` | `float32` | 保留，置 0 |
| 13–16 | `angular_x` | `float32` | 底盘旋转角速度（rad/s）。当前实现：置 0 保留 |
| 17–20 | `angular_y` | `float32` | 云台 pitch 角度指令（rad） |
| 21–24 | `angular_z` | `float32` | 云台 yaw 角度指令（rad） |
...
| 4–7 | `linear_x` | `float32` | 底盘前后速度（m/s） |
| 8–11 | `linear_y` | `float32` | 底盘左右速度（m/s） |
| 12–15 | `linear_z` | `float32` | 保留 |
| 16–19 | `gyro_wz` | `float32` | 底盘旋转角速度（rad/s） |
| 20–23 | `angular_y` | `float32` | 云台 pitch 角度（rad） |
| 24–27 | `angular_z` | `float32` | 云台 yaw 角度（rad） |
