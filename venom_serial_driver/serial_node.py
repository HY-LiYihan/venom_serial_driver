...
from venom_serial_driver.msg import RobotStatus, GameStatus, AutoAimCmd
import time
...
class SerialDriverNode(Node):
    def __init__(self):
        ...
        self.auto_aim_sub = self.create_subscription(
            AutoAimCmd, '/auto_aim', self._auto_aim_callback, 10)
    ...
    def _auto_aim_callback(self, msg: AutoAimCmd) -> None:
        """Cache the latest auto-aim command."""
    ...
    def _send_ctrl_frame(self) -> None:
        ...
        if aim is not None:
            ctrl.ay = float(aim.pitch)
            ctrl.az = float(aim.yaw)
